...
//Function that allows to set task parameters.
...
//Calculate the total time or period the tasks are going to run.
...
//Identify the presence of a task that match current timer, this means it is ready.
...
//Calculate the CPU utilization, this determines if the tasks can be scheduled or not.
